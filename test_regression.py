...
class TestEveryTestFileIsCollected:
    """`pytest.ini` names test files explicitly, so a new one is not run.

    That is not hypothetical: test_telemetry.py sat on disk with 26 passing
    tests that bare `pytest` never collected. A test that does not run is
    worse than no test -- it looks like coverage on the file listing and
    catches nothing.
    """

    def test_no_test_file_is_left_out_of_testpaths(self):
        import configparser
        import pathlib

        here = pathlib.Path(__file__).parent
        config = configparser.ConfigParser()
        config.read(here / "pytest.ini")
        listed = set(config["pytest"]["testpaths"].split())
        on_disk = {p.name for p in here.glob("test_*.py")}

        missing = on_disk - listed
        assert not missing, (
            f"{sorted(missing)} exist but are not in pytest.ini testpaths, so "
            "bare `pytest` does not run them"
        )

    def test_testpaths_names_no_file_that_is_gone(self):
        import configparser
        import pathlib
        ...
        config.read(here / "pytest.ini")
        listed = set(config["pytest"]["testpaths"].split())
        missing = {name for name in listed if not (here / name).exists()}
        assert not missing, f"pytest.ini names files that do not exist: {missing}"
